...
## Numeric input expressions, check identities and owner rollover contract (18 September)

Root cause: literal templates treated the sign of a numeric operand and the component count of a constant-only source composition as model wiring, rejecting valid disclosed replacements. Generic rule: numeric signs belong to numeric inputs; expressions with no model references may change their arithmetic composition, while references, reference signs and operators around live model references remain protected. Source evidence is still verified or flagged. Pins: tests/test_signed_operands.py, including a pure output that cannot become a numeric composition. A fresh local original-workbook/page166 trial updates net finance constants correctly in one call (2,471 tokens). A preceding wrong-page trial correctly refused and used2,607 tokens; neither trial is full-model acceptance.

Root cause: runtime anatomy converted any numeric row into a zero-balance objective without the evidence used at discovery. Generic rule: promoting a check requires its own accounting-identity explanation, a live formula, and the model's historical reconciliation evidence. Cold discovery and runtime anatomy share the validator. Financial-key declarations use the existing role vocabulary rather than arbitrary operational metrics. Pins: tests/test_anatomy_evidence.py. The museum's former promotion of an operating-profit sum into a zero check was incorrect; its fixture now demonstrates an actual balance identity.

Owner's explicit rollover exceptions: freeze the next forecast period's linked percentage assumption at its pre-update value; retain its pre-update zero forecasts. Unknown/absent/circular values are not zero. Accounting check rows are measurements, not forecasts, and remain live. The actual next period comes from the model axis, not physical adjacency; later formulas remain live. Holds pass through the writer, are read back, recorded as authorized freezes and protected from subsequent repair writes. Pins: tests/test_freeze_rollover_rules.py.

Owner's remaining rules are retained in the mapping contract: historical restatement is a question awaiting analyst approval; use prior-report triangulation when comparatives changed. Reclassified segments require matching name AND prior value for direct mapping; otherwise derive a flagged estimate using the relevant total's growth. The production reportpage renderer now retains only the What's changed table, including its live new values, old snapshot, deltas and in-table print evidence. Removed page sections do not erase cell flags or evidence artifacts. Pin: tests/test_compact_report.py and updated public production museum fixture.

Full regression, three pinned replays and affected-cell readiness are pending. No live dispatch until mandatory gates pass.

Rejected experiment: automatically solving an actual input from a shifted forecast equation worsened the CLP replay (first-forecast balance gap 529 -> 830 and net profit changed). The experiment has been removed, not committed. The root ambiguity is the scope of the owner zero-forecast rule: calculated opening balances and accounting links can evaluate to zero before update without being zero assumptions or one-offs. Preserving every such zero freezes the accounting chain. The owner subsequently clarified the scope below; no waiver of the mandatory balanced replay gate is claimed. Rejected experiment retained outside the repository at /private/tmp/codex-rejected-rollbase.patch for diagnosis.

## Owner clarification: one-off events, not every computed zero (18 September, morning)

The owner explicitly limited zero preservation to one-off items, exemplified by CLP's discrete capacity change. Installed capacity should retain the actual change; the addition or retirement must not repeat in originally-zero forecast years. This resolves the earlier scope question. The overnight pause is no longer a blocker.

Root cause: the freeze planner inferred economic non-recurrence from a computed zero, converting accounting formulas into constants. Generic evidence rule: the brain identifies the discrete event input and explains its meaning; code verifies the original forecast was zero, the row is an input rather than an accounting output, and the affected forecast expression carries that same input. Preserve all originally-zero future event forecasts, leave originally nonzero forecasts unchanged, and keep totals/opening balances live. Growth-assumption freezes remain separate. No company label or coordinate is embedded in the implementation. Pins: tests/test_freeze_rollover_rules.py and tests/test_oneoff_mapping_contract.py. Validation pending; no new live dispatch yet.

Current morning candidate: full codex_validate bench GREEN; all three pinned replays delivered with balance status pass (CLP FY25, DFE FY25, DFE 1H25). Pipeline hashes match /private/tmp/codex-event-final-build/manifest.json. Exact-candidate affected-input readiness is running. These are offline engineering checks, not cold accuracy or native Excel acceptance.

Rotated/PPE audit: existing displayed-orientation and six-table-kind museum tests pass without skips on the supplied DFE annual report. Pages 184–185 are rotated 270 degrees and contain gross cost, accumulated depreciation, impairment and carrying values by asset category. Production rotation logic matches the Claude checkout. Reading this matrix is proven; full mapping of complex PPE tables is not established by those extraction tests. A focused fresh-context mapping trial is separate evidence.

Root cause: the verified-printed-quote branch refused unresolved scale before considering the explicit model-unit value it asked the brain to provide. Generic evidence rule: a verified source quote plus finite explicit model-unit value and explanation can land RED when comparative scale is unresolved; incompatible-unit guards and missing-quote refusals remain intact. Pin: tests/test_mapping_model_units.py. Actual rotated PPE saved-answer delivery is replayed offline; no company-specific coordinates or numeric values enter production. Final combined gates must be rerun before commit.

The same unit boundary also ignored explicit model-unit values when an inferred automatic conversion existed. A verified quote with a conflicting explicit model-unit value now retains the explained value RED and records both conversions; it does not silently substitute an inferred scale. The focused conflict regression is included in tests/test_mapping_model_units.py. Combined final candidate is pinned at /private/tmp/codex-unit-final-build/manifest.json; final gates pending.

Final combined candidate passes the full codex_validate suite, all three balanced pinned replays (CLP 24 checks; DFE FY25 118; DFE 1H25 84), and all five targeted delivery checks with one candidate each: CLP Driver AI103 / Final AI21, DFE Raw financials U68 / U93 / U50. Exact hashes, logs and scope limitations: validation/owner-rules/unit-final-gates.json. This clears engineering dispatch gates only, not full-model key accuracy or native Excel certification.
