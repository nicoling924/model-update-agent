...
model's own sign convention). `value` is explicitly in the model's units; explain the conversion or
derivation in `because`. If row units conflict with a page's monetary scale, give a model-unit `value`
and explain the row's units. It stays flagged for review; never force the monetary scale onto that row.
...
def _face_rows(loop, pre_wb, rows, skipped):
    ...
    old = _other_period_docs(loop)
    faces = [(d, p) for _f, d, p in _faces(loop)
             if d not in old and page_of_text(loop, d, p)]
...
def route_open_rows(loop, pre_wb, rows, page_text, skipped, ask_json, log, deadline=None,
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        try:
            ...
            continue
        log("[map] routing reply %s" % json.dumps(reply, ensure_ascii=False))
...
class Pages:
    ...
    def __init__(self, docs, later_docs=(), log=print, ledger=None):
        self._d, self._later, self._log, self._loaded = {}, list(later_docs or ()), log, False
        for p in docs or ():
            self._read(p)
        # The page tool and quote verifier must see the same accepted scan
        # evidence as the ledger. Native page text remains authoritative;
        # disputed OCR is never presented as verified page text.
        scans = {}
        for item in getattr(ledger, "items", ()):
            if item.channel != "vision" or item.disputed or not item.source_line:
                continue
            scans.setdefault((item.doc, item.page), []).append(item)
        for key, items in scans.items():
            if not self._d.get(key):
                ordered = sorted(items, key=lambda it: (it.table_id or 0, it.row_ord or 0))
                self._d[key] = "\n".join(it.source_line for it in ordered)
...
def page_text_of_docs(docs, later_docs=(), log=print, ledger=None):
    """{(doc, page): text} — this period's pages now, earlier ones on demand."""
    return Pages(docs, later_docs, log, ledger=ledger)
