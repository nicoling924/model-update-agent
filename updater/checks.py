...
def summarize(card, target_year, flags=None, spec=None, wb=None):
    """One-screen scorecard text for the agent loop's state block.

    NOTE (objectives rework 2026-08-17): there is NO flag budget. Flags are
    "look here" routing for the analyst, never a delivery meter — the old
    >15% arithmetic is dead doctrine (BOSS_MINDMAP.md). The loop still sees
    the flag COUNT so it prefers proven values over flags, but a flag total
    never fails anything.
    """
    lines = []
    if flags:
        lines.append(f"FLAGS: {len(set(flags))} cells marked 'look here' "
                     "(fewer is better; never a blocker)")
