...
# Usage: sh dispatch.sh [COMPANY] [PERIOD] [PRIOR_PERIOD] [BRANCH] [ACTION]
#   ACTION: chain (legacy) or pipeline (the clean rebuild)
COMPANY="${1:-CLP}"; PERIOD="${2:-FY25}"; PRIOR="${3:-FY24}"; BRANCH="${4:-objective-driven}"; ACTION="${5:-chain}"
...
CODE=$(curl -s -o /tmp/dispatch_resp.txt -w "%{http_code}" -X POST \
  ...
  -d "{\"ref\":\"$BRANCH\",\"inputs\":{\"company\":\"$COMPANY\",\"period\":\"$PERIOD\",\"prior_period\":\"$PRIOR\",\"action\":\"$ACTION\"}}")
if [ "$CODE" = "204" ]; then
  echo "DISPATCHED: $COMPANY $PERIOD $ACTION on branch $BRANCH"
